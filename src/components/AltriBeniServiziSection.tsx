  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('altri_beni_servizi').select('*').eq('preventivo_id', preventivoId).order('created_at', {
        ascending: true
      });
  ...
  return <div className="w-full">
      ...
      <div className="w-full border rounded-b-lg overflow-hidden">
        <Table className="w-full">
          ...
          <TableBody>
            {items.map((item, index) => <TableRow key={item.id ?? `tmp-${index}`} className="hover:bg-hsl(var(--section-services))/20">
                <TableCell className="text-center text-sm text-muted-foreground">{index + 1}</TableCell>

                <TableCell>
                  <Input value={item.descrizione} onChange={e => updateItem(index, 'descrizione', e.target.value)} placeholder="Descrizione del bene/servizio" className="border-0 bg-transparent focus:bg-white/50 resize-none min-h-[60px]" />
                </TableCell>
              ...
              }} className="border-0 bg-transparent focus:bg-white/50 text-center" step="0.01" min="0" />
                </TableCell>
              ...
              }} className="border-0 bg-transparent focus:bg-white/50 text-center" step="1" min="0" max="200" />
                </TableCell>
                ...
                <TableCell>
                  {items.length > 1 && <Button onClick={() => removeItem(index)} size="sm" variant="ghost" className="h-8 w-8 p-0 text-destructive hover:text-destructive hover:bg-destructive/10">
