...
-- Policies for profiles (drop then create)
...
create or replace trigger update_parametri_updated_at
...
create or replace trigger update_costi_retroilluminazione_updated_at
...
create or replace trigger update_listino_accessori_stand_updated_at
before update on public.listino_accessori_stand
for each row execute function public.update_updated_at_column();

create or replace trigger update_listino_accessori_desk_updated_at
before update on public.listino_accessori_desk
for each row execute function public.update_updated_at_column();

create or replace trigger update_listino_accessori_espositori_updated_at
...
create or replace trigger update_costi_struttura_desk_layout_updated_at
before update on public.costi_struttura_desk_layout
for each row execute function public.update_updated_at_column();

create or replace trigger update_costi_struttura_espositori_layout_updated_at
