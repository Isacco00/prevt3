...
-- Disallow public inserts by default; only service role may insert via edge function
-- This policy allows inserts only when using service role (role claim = service_role)
...
-- Policies for parametri
create policy if not exists "Authenticated can read parametri"
  on public.parametri for select
  using (auth.uid() is not null);

create policy if not exists "Admins can modify parametri"
...
create policy if not exists "Authenticated can read costi_retroilluminazione"
  on public.costi_retroilluminazione for select
  using (auth.uid() is not null);

create policy if not exists "Admins can modify costi_retroilluminazione"
...
create policy if not exists "Authenticated can read listini accessori stand"
  on public.listino_accessori_stand for select
  using (auth.uid() is not null);
create policy if not exists "Authenticated can read listini accessori desk"
  on public.listino_accessori_desk for select
  using (auth.uid() is not null);
create policy if not exists "Authenticated can read listini accessori espositori"
  on public.listino_accessori_espositori for select
  using (auth.uid() is not null);

create policy if not exists "Admins can modify listino accessori stand"
  on public.listino_accessori_stand for all
  using (public.is_admin()) with check (public.is_admin());
create policy if not exists "Admins can modify listino accessori desk"
  on public.listino_accessori_desk for all
  using (public.is_admin()) with check (public.is_admin());
create policy if not exists "Admins can modify listino accessori espositori"
...
create policy if not exists "Authenticated can read costi struttura desk"
  on public.costi_struttura_desk_layout for select
  using (auth.uid() is not null);
create policy if not exists "Authenticated can read costi struttura espositori"
  on public.costi_struttura_espositori_layout for select
  using (auth.uid() is not null);

create policy if not exists "Admins can modify costi struttura desk"
  on public.costi_struttura_desk_layout for all
  using (public.is_admin()) with check (public.is_admin());
create policy if not exists "Admins can modify costi struttura espositori"
